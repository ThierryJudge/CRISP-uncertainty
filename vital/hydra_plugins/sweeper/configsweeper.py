...
@dataclass
class ConfigSweeperConf:
    _target_: str = "hydra_plugins.sweeper.configsweeper.ConfigSweeper"
    max_batch_size: Optional[int] = None
    overrides: Dict[str, str] = field(default_factory=dict)


ConfigStore.instance().store(group="hydra/sweeper", name="config", node=ConfigSweeperConf)


log = logging.getLogger(__name__)


class ConfigSweeper(Sweeper):
    """
    Basic sweeper
    """

    def __init__(self, max_batch_size: Optional[int], overrides: Dict[str, str]) -> None:
        """
        Instantiates
        """
        super(ConfigSweeper, self).__init__()
    ...
    def split_arguments(overrides: List[Override], max_batch_size: Optional[int]) -> List[List[List[str]]]:
            ...
            if override.is_sweep_override():
                if override.is_discrete_sweep():
                    ...
                    raise HydraException(
                        f"{ConfigSweeper.__name__} does not support sweep type : {override.value_type.name}"
                    )
            else:
                key = override.get_key_element()
        ...
        if max_batch_size is None:
            ...
            chunks_iter = ConfigSweeper.split_overrides_to_chunks(all_batches, max_batch_size)
    ...
    def sweep(self, arguments: List[str]) -> Any:
        ...
        configured_arguments = [f"{key}={val}" for key, val in self.configured_overrides.items()]
